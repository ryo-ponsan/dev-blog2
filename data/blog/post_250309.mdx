...
GitHub でリポジトリを Fork すると、オリジナルのリポジトリ（以下「Fork 元」）をコピーして自分のリポジトリ（以下「Fork 先」）として管理できる。しかし、時間が経つと Fork 元のリポジトリが更新され、Fork 先との差分が生じることがある。

本記事では、Fork 元の最新の内容を Fork 先に反映する方法を、`merge` と `rebase` の 2 つの方法で詳しく解説する。

---

## 1. Fork 元のリポジトリをリモートリポジトリとして追加

まず、Fork 元のリポジトリを `upstream` という名前でリモートリポジトリとして登録する。

```sh
# Fork 元のリポジトリを upstream として追加
